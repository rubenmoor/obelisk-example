...
import           Common.Model                   (Rank (..), JournalEvent(..), EventUser(..) )
...
handleUserNew UserNew {..} = do
    ...
    now <- liftIO getCurrentTime
    keyAlias <- runDb $ insert $ Db.Alias sdAliasName user now unVisible
    ...
    sdJwt <-
        liftIO (runExceptT $ mkCompactJWT jwk claims)
            >>= either toServerError pure
    let
        sdClearances = Map.empty
        sdUserName = unUserName
